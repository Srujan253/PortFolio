...
import { FaGithub, FaExternalLinkAlt, FaReact, FaNodeJs, FaTimes, FaCheckCircle, FaLaptopCode, FaRocket } from "react-icons/fa";
import {
  SiTailwindcss,
  SiExpress,
  SiMongodb,
...
const projects = [
  {
    ...
    fullDescription: "A comprehensive full-stack club management platform designed to solve fragmented communication. It enables users to invite members, join specialized communities, and collaborate seamlessly via real-time chat with complete CRUD capabilities.",
    features: ["Role-Based Access Control", "Real-Time Community Chat", "Dynamic Event Scheduling", "Analytics Dashboard"],
    challenge: "Handling concurrent real-time connections efficiently while ensuring strict data isolation between different clubs.",
  ...
  },
  {
    ...
    fullDescription: "GupShup is a robust real-time messaging platform built on the MERN stack and Socket.io. It features instant message delivery, online presence indicators, and AI-powered chat suggestions to keep conversations engaging.",
    features: ["Instant Messaging via WebSockets", "AI Message Suggestions", "Online/Offline Presence", "Media Sharing"],
    challenge: "Optimizing MongoDB queries for loading chat history instantly without causing lag during rapid message bursts.",
  ...
  },
  {
    ...
    fullDescription: "A privacy-first identity verification web application that generates cryptographically signed QR codes. It supports offline verification via RSA signatures and online verification using JWTs, ensuring user data remains protected.",
    features: ["RSA Signed QR Codes", "JWT Online Verification", "Zero-Knowledge Proofs", "Offline Mode"],
    challenge: "Implementing robust RSA cryptography in the browser while maintaining high performance for QR generation.",
  ...
  },
  {
    ...
    fullDescription: "A sleek marketplace designed for architects and digital artists. Features include role-based authentication, advanced artwork filtering, AI-powered descriptions, and highly animated UI components for a premium browsing experience.",
    features: ["Role-Based Auth", "Advanced Search & Filtering", "AI Descriptions", "Animated UI"],
    challenge: "Designing a highly complex animated UI that performs smoothly at 60fps across all devices.",
  ...
  },
  {
    ...
    fullDescription: "A highly creative, interactive personal portfolio inspired by the aesthetics and mechanics of Clash Royale. It gamifies the browsing experience, turning standard sections into 'arenas' and 'cards'.",
    features: ["Gamified UI", "Custom Animations", "Interactive 'Cards'", "Themed Sound Effects"],
    challenge: "Replicating complex game mechanics and animations strictly using web technologies and CSS.",
    image: "https://placehold.co/800x500/0f172a/06b6d4?text=Clash+Royale+Portfolio",
    tech: [
      { icon: <FaReact />, name: "React", color: "text-cyan-400" },
      { icon: <SiTailwindcss />, name: "Tailwind CSS", color: "text-cyan-300" },
      { icon: <SiVite />, name: "Vite", color: "text-purple-400" },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <p className="text-lg text-gray-400 max-w-2xl mx-auto">
            A curated selection of my latest work. Click on any project to dive into the detailed case study.
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 auto-rows-[250px] mb-12">
          {displayedProjects.map((project, index) => (
            ...
            >
              <motion.div className="absolute inset-0 w-full h-full" layoutId={`image-container-${project.id}`}>
                <img 
                  src={project.image} 
                  alt={project.title} 
                ...
                <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                  <div className="flex gap-2 mb-3">
                    {project.tech.slice(0, 3).map((t, i) => (
                      <span key={i} className="bg-black/50 backdrop-blur-md p-2 rounded-lg border border-white/10">
                        {t.icon}
                      </span>
                    ))}
                  </div>
                ...
                <div className="max-w-xl mx-auto w-full">
                  ...
                  <div className="space-y-8">
                    ...
                    <div>
                      ...
                      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {selectedProject.features.map((feature, i) => (
                          <li key={i} className="flex items-center gap-3 text-gray-400 bg-white/5 px-4 py-2 rounded-xl border border-white/5">
                    ...
                    <div className="bg-gradient-to-r from-blue-900/20 to-transparent p-6 rounded-2xl border-l-4 border-blue-500">
                      <h4 className="text-lg font-bold text-blue-400 mb-2">The Engineering Challenge</h4>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-3">
                        {selectedProject.tech.map((t, i) => (
                          <div key={i} className="flex items-center gap-2 px-4 py-2 bg-gray-900 rounded-full border border-gray-800 hover:border-cyan-500/50 transition-colors group cursor-default shadow-lg">
                            <span className={`text-xl ${t.color}`}>{t.icon}</span>
                            <span className="text-gray-300 text-sm font-semibold group-hover:text-white transition-colors">{t.name}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-4 pt-4 border-t border-white/10">
                      ...
                      >
                        <FaExternalLinkAlt /> {selectedProject.demo !== "#" ? "Live Demo" : "Offline"}
                      </a>
                      ...
                      >
                        <FaGithub className="text-xl" /> {selectedProject.github !== "#" ? "Source Code" : "Private"}
                      </a>
                    </div>
                  </div>
                </div>
